####
# - Main purpse is to Follow a Wall on right side by using LIDAR sensor  
# - Wall on the right is is followed at a fixed distance by using proportional controller
...
class follow_wall_bot(Node):
    ...
    def get_scan_values(self,scan_data):
        ...
        self.region_4= min(min(scan_data.ranges[165:195]), 100 )
        ...
        print(round(self.region_3,3) ,"/",round(self.region_2,3),"/",round(self.region_1,3),"\n", 
              "Reg_4:",round(self.region_4,3),"/",round(self.velocity.angular.z,3),"/",self.case )
    ...
    def proporitonal_control(self):
        ...
        self.velocity.angular.z= k_p*(self.error)
    ...
    def send_cmd_vel(self):
        ...
        if(self.region_1<4): ## last ray block is on the wall
            ...
            if(self.region_4 < 1.5):
                self.case = "Obstacle Ahead"
                self.velocity.linear.x= 0.34 
                self.velocity.angular.z= 1.57 ## left turn
